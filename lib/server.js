...
async function createAdminListing(input) {
  ...
  if (title.length > 2000) {
    throw createHttpError(400, "Başlık çok uzun (en fazla 2000 karakter).");
  }
  ...
  const properties = {
    ...
    Price: richTextProperty(String(input?.price || "").trim().slice(0, 2000)),
    Location: richTextProperty(String(input?.location || "").trim().slice(0, 2000)),
    Rooms: richTextProperty(String(input?.rooms || "").trim().slice(0, 2000)),
    Bathrooms: richTextProperty(String(input?.bathrooms || "").trim().slice(0, 2000)),
    Area: richTextProperty(String(input?.area || "").trim().slice(0, 2000)),
    Description: richTextProperty(String(input?.description || "").trim().slice(0, 2000)),
    Published: { checkbox: input?.published === false ? false : true },
    Featured: { checkbox: false },
  };

  if (input?.category) {
    properties.Category = { select: { name: String(input.category).slice(0, 100) } };
  }
  if (input?.status) {
    properties.Status = { select: { name: String(input.status).slice(0, 100) } };
  }
  if (images.length) {
...
if (require.main === module) {
  ...
  server.listen(PORT, HOST, () => {
    console.log(`Likya World Gayrimenkul site running at http://localhost:${PORT}`);
  });
}
...
function mapGlobalSiteSettings(properties) {
  return {
    brandName: readRichText(properties.BrandName) || "Likya World Gayrimenkul",
...
async function sendLeadWithResend(to, subject, text) {
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: process.env.RESEND_FROM_EMAIL || "Likya World Gayrimenkul <onboarding@resend.dev>",
      to,
      subject,
      text,
    }),
...
async function sendLeadWithNodemailer(to, subject, text) {
  ...
  await transporter.sendMail({
    from: process.env.SMTP_FROM_EMAIL || process.env.RESEND_FROM_EMAIL || "Likya World Gayrimenkul <no-reply@localhost>",
    to,
    subject,
    text,
  });
